<template>
  <form class="form  p-b-50 font-28 bgG">
    <div class="bgW p-t-20 p-l-40 p-r-40">
      ...
      <div class="item">
        <label>地址：</label>
        <!--<mu-text-field hintText="省/市/县" v-model="form.planAddress" @focus="areaFlag=true;$event.target.blur()" :error-text="formRule.planAddress.text" v-if="0"/>-->
        <mu-text-field v-model="form.planDetailaddress" hintText="选择地址" :error-text="formRule.planDetailaddress.text" @focus="pmapFlag=true;$event.target.blur()" multiLine/>
      </div>
      <div class="item">
        <label>开始时间：</label>
  ...
  import popupMap from '../util/pmap.vue'
    export default {
      ...
      watch:{
        ...
        'form.planDetailaddress'(val){
          //this.form.planAddress=val
          this.checkfun('planDetailaddress')
        }
      },
      methods: {
        addPro() {
        ...
        //获取项目列表
        getDictList(){
          this.$ajax.post('/project/dictList',{})
            .then(result=>{
        ...
        submit(){
          ...
          this.$ajax.post('/plan/add',this.form)
            .then(result=>{
              this.$toast({
                message: '创建成功',
